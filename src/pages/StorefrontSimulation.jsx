...
import { Search, Filter, MapPin, Calendar, Star, ArrowRight, Info } from 'lucide-react';
...
const StorefrontSimulation = () => {
    ...
    const [filterType, setFilterType] = useState('all'); // all, umrah, haji, tour

    useEffect(() => {
        const fetchPackages = async () => {
            try {
                // Ambil data paket yang statusnya 'active'
                        ...
                        filteredPackages.map((pkg) => (
                            <div key={pkg.id} className="group bg-white rounded-2xl border border-gray-200 overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col h-full">
                                ...
                                <div className="h-48 bg-gray-200 relative overflow-hidden">
                                    ...
                                    {/* Placeholder Image - Bisa diganti image dinamis nanti */}
                                    <img 
                                    ...
                                    <div className="absolute top-3 left-3 z-20">
                                        <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide text-white shadow-sm ${
                                            pkg.type === 'haji' ? 'bg-purple-600' : pkg.type === 'tour' ? 'bg-green-600' : 'bg-blue-600'
                                        }`}>
                                            {pkg.type.replace('_', ' ')}
                                        </span>
                                    </div>
                                ...
                                <div className="p-5 flex flex-col flex-1">
                                    ...
                                    {/* Fasilitas Tags (Static Mockup) */}
                                    <div className="flex flex-wrap gap-2 mb-6">
                                        <span className="text-[10px] bg-green-50 text-green-700 px-2 py-1 rounded border border-green-100">Tiket PP</span>
                                        <span className="text-[10px] bg-orange-50 text-orange-700 px-2 py-1 rounded border border-orange-100">Visa</span>
                                        <span className="text-[10px] bg-blue-50 text-blue-700 px-2 py-1 rounded border border-blue-100">Muthawif</span>
                                    </div>
                                    ...
                                    <div className="mt-auto pt-4 border-t border-gray-100">
                                        ...
                                        <div className="flex justify-between items-end">
                                            <div className="text-xl font-bold text-blue-700">
                                                {formatPrice(pkg.base_price_quad)}
                                            </div>
                                        ...
                                        <p className="text-[10px] text-gray-400 mt-1 text-right">Per pax (Quad)</p>
                                    </div>
                                </div>
                            </div>
                        ))
