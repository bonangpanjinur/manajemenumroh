...
const Finance = () => {
    const { data, loading, fetchData, createItem, updateItem, deleteItem } = useCRUD('umh/v1/payments');
    ...
    const handleOpenModal = (mode, item = null) => {
        ...
        if (item && item.jamaah_id) {
             const detail = jamaahList.find(j => String(j.id) === String(item.jamaah_id));
    ...
    const handlePrintReceipt = (id) => {
        if (!window.umhData?.siteUrl) {
            alert("Error: Base URL tidak ditemukan.");
            return;
        }
    ...
    return (
        <Layout title="Keuangan & Kasir">
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={modalMode === 'create' ? "Catat Pembayaran" : "Edit Transaksi"} size="max-w-lg">
                <form onSubmit={handleSubmit} className="space-y-5">
                    
                    <div>
                        <label className="label">Cari Jemaah</label>
                        <select className="input-field" value={formData.jamaah_id} onChange={handleSelectJamaah} required disabled={modalMode === 'edit'}>
                    ...
                    {selectedJamaahDetail && (
                        <div className="bg-blue-50 p-4 rounded-lg text-sm text-blue-900 border border-blue-200">
                            ...
                            <div className="flex justify-between"><span>Total Tagihan:</span> <span className="font-bold">{formatCurrency(selectedJamaahDetail.package_price || 0)}</span></div>
                        </div>
                    )}
